...
# Terminal 1: kuavo down --> 
'''
cd kuavo-ros-opensource && sudo su
source devel/setup.bash
roslaunch humanoid_controllers load_kuavo_real.launch joystick_type:=h12

after calibration, press 'o'
'''

# Terminal 2: source devel/setup.bash --> python3 src/my_controller/kuavo_arm_capture_apriltag.py
...
class Controller():
    ...
    # Head 
    def set_head_action(self, target_yaw, target_pitch):
        pass
...
def set_head_target(yaw, pitch):
    """
    设置头部目标位置，并发布消息
    :param yaw: 头部旋转，范围为[-30, 30]度, max [-90, 90]
    :param pitch: 头部俯仰，范围为[-25, 25]度, max [-30, 30]
    """
    ...
    head_target_msg.joint_data = [yaw, pitch] # TODO: check whether yaw pitch or pitch yaw???
...
def main():
    ...
    set_head_target(yaw=0, pitch=20)
    print(f"head down")
    while not rospy.is_shutdown():
        processor.get_apriltag_data()
    set_head_target(0, 0)
    time.sleep(2)
...
#     elif start_with_version(robot_version, 45):
#         robot_zero_x = -0.0175
