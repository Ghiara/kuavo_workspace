...
def warn_missing_topics(missing_topics, message):
    for topic in missing_topics:
        console.print(f"[yellow]WARN: {message}: {topic}[/yellow]")


def check_critical_topics_before_recording():
    try:
        ...
        console.print(f"[yellow]WARN: failed to query ROS topics before recording: {e}[/yellow]")
        return

    available_topics = set(result.stdout.splitlines())
    missing_topics = [topic for topic in CRITICAL_IMAGE_TOPICS if topic not in available_topics]
    warn_missing_topics(missing_topics, "critical image topic is not available before recording")
...
def record_rosbag(config):
    try:
        ...
        check_critical_topics_before_recording()
